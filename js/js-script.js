	...
	var webistleio = {
		...
		Basic: {
			init: function() {
				...
				this.webistleHeader();
				this.webistleSideInner();
				this.webistleMobileMenu();
			...
			EiStickyMenu: function (){
				...
				$('.appi-ab-open_mobile_menu').on("click", function() {
					$('.appi-ab-mobile_menu_wrap').toggleClass("mobile_menu_on");
				});
				$('.appi-ab-open_mobile_menu').on('click', function () {
					$('body').toggleClass('mobile_menu_overlay_on');
				});
				if($('.appi-ab-mobile_menu li.dropdown ul').length){
					$('.appi-ab-mobile_menu li.dropdown').append('<div class="dropdown-btn"><span class="fa fa-angle-down"></span></div>');
					$('.appi-ab-mobile_menu li.dropdown .dropdown-btn').on('click', function() {
			...
			EiscreenSlider: function (){
				if ($('.ab-appScreenshotCarousel-container').length) {
					if ($('.ab-appScreenshotCarousel-container').length) {
						var swiper = new Swiper('.ab-appScreenshotCarousel-container', {
							effect: 'coverflow',
							loop: true,
							centeredSlides: true,
							slidesPerView: 4,
			...
			faqBg: function (){
				$(document).on('click', '.ab-faq', function(){
			...
			EiServiceSLider: function (){
				if($('.ab-service-slide').length){
					var customPagerSlider = $(".ab-service-slide").bxSlider({
						adaptiveHeight: true,
						controls: true,
						pause: 5000,
						speed: 1000,
			...
			EiPartnerSlider: function (){
				if($('#ab-partner-slide').length){
					$('#ab-partner-slide').owlCarousel({
						margin: 105,
						autoplay: true,
						loop: true,
						responsiveClass:true,
			...
			webistleHeader: function (){
				jQuery(window).on('scroll', function() {
					if (jQuery(window).scrollTop() > 100) {
						jQuery('.webistle-main-header').addClass('webistle-sticky-header-overlay')
					} else {
						jQuery('.webistle-main-header').removeClass('webistle-sticky-header-overlay')
					}
				})
			},
			webistleSideInner: function (){
			...
			webistleMobileMenu: function (){
				$('.webistle-open_mobile_menu').on("click", function() {
					$('.webistle-mobile_menu_wrap').toggleClass("mobile_menu_on");
				});
				$('.webistle-open_mobile_menu').on('click', function () {
					$('body').toggleClass('mobile_menu_overlay_on');
				});
				if($('.webistle-mobile_menu li.dropdown ul').length){
					$('.webistle-mobile_menu li.dropdown').append('<div class="dropdown-btn"><span class="fa fa-angle-down"></span></div>');
					$('.webistle-mobile_menu li.dropdown .dropdown-btn').on('click', function() {
						$(this).prev('ul').slideToggle(500);
					});
				}
			},
			...
			webistleTestimonial: function (){
				$('#webistle-testimonial-id').slick({
					slidesToShow: 1,
					slidesToScroll: 1,
					arrows: false,
					fade: true,
					asNavFor: '#testimonial-webistle-name'
				});
				$('#testimonial-webistle-name').slick({
					slidesToShow: 3,
					slidesToScroll: 1,
					asNavFor: '#webistle-testimonial-id',
					dots: true,
					centerMode: true,
					focusOnSelect: true
				});
