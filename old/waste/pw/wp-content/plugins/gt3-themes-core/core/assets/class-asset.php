...
abstract class Asset {
	...
	protected function init(){
		...
		add_action('wp_enqueue_scripts', array( $this, 'process_header' ), 100);
	...
	protected function init_params(){
		...
		$this->asset_theme['path'] = sprintf('%sdist/%s/', trailingslashit(get_stylesheet_directory()), $this->sub_folder);
		$this->asset_theme['url']  = sprintf('%sdist/%s/', trailingslashit(get_stylesheet_directory_uri()), $this->sub_folder);

		if($this->is_child_theme) {
			$this->asset_child['path'] = sprintf('%sdist/%s/', trailingslashit(get_template_directory()), $this->sub_folder);
			$this->asset_child['url']  = sprintf('%sdist/%s/', trailingslashit(get_template_directory_uri()), $this->sub_folder);
		}
	}
	...
	public static function register_widget($widget_name, $depts = array()){
		...
		if(Assets::$is_preview) {
			self::enqueue_widget($widget_name);
		}
	}
	...
	public static function register_core_asset($handle, $deps = array()){
		...
		return static::register_asset($handle, $deps, $url, $path);
	}
	...
	public static function enqueue_core_asset($handle, $deps = array(), $url = ''){
		...
		if(!in_array($handle, $self->done) && !in_array($handle, $self->enqueued)) {
			...
			return $self->enqueue_asset(static::CORE.'/'.$handle);
		}
	...
	public static function enqueue_theme_asset($handle, $deps = array(), $url = false){
		...
		if(!in_array($handle, $self->done) && !in_array($handle, $self->enqueued)) {
			...
			return $self->enqueue_asset(static::THEME.'/'.$handle);
		}
	...
	public static function enqueue_child_asset($handle, $deps = array(), $url = ''){
		...
		if(!in_array($handle, $self->done) && !in_array($handle, $self->enqueued)) {
			...
			return $self->enqueue_asset(static::CHILD.'/'.$handle);
		}
	...
	public function hash(){
		...
		foreach($this->enqueued as $asset) {
			list ('ver' => $ver) = $this->register[$asset];
			$name[] = $asset.'~'.$ver;
		}
	...
	public function save_file($assets, $path, $before = '', $after = ''){
		...
		foreach($assets as $asset) {
			list ('path' => $f) = $this->register[$asset];
			$FH = fopen($f, "r");
			fputs($stream, $before);
